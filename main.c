...
int main(int argc, char *argv[])
{
  ...
  case 'm':
    ...
    if (!(srv.map = reallocarray(srv.map, ++srv.map_len, sizeof(struct map))))
    {
      die("reallocarray:");
    }
  ...
  case 'u':
    ...
    break;
  case 'v':
    if (spacetok(EARGF(usage()), tok, 4) || !tok[0] || !tok[1] || !tok[2])
    {
      usage();
    }
    if (!(srv.vhost =
              reallocarray(srv.vhost, ++srv.vhost_len, sizeof(*srv.vhost))))
    {
      die("reallocarray:");
    }
  ...
  {
    die("UNIX-domain socket '%s': %s", udsname,
        errno ? strerror(errno) : "File exists");
  }
  ...
  for (i = 0; i < srv.vhost_len; i++)
  {
    ...
    {
      die("regcomp '%s': invalid regex", srv.vhost[i].regex);
    }
  }
